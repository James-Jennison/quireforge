...
class NodeAuditExceptionTests(unittest.TestCase):
    def test_clean_audit_is_accepted_without_exceptions(self): self.assertEqual(validate_report({"advisories": {}}, []), [])
    def test_clean_audit_rejects_any_new_advisory(self): self.assertTrue(validate_report(REPORT, []))
