...
class CmakeOptionsTests(unittest.TestCase):
    ...
    def test_rejects_a_tar_archive_with_an_oversized_member_name(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            source = Path(temporary_directory)
            readme = source / "readme.txt"
            readme.write_text("source only", encoding="utf-8")
            with tarfile.open(source / "archive.tar", "w") as archive:
                archive.add(
                    readme,
                    arcname="a" * (VALIDATOR.MAX_ARCHIVE_MEMBER_NAME_BYTES + 1),
                )
            readme.unlink()

            with self.assertRaisesRegex(SystemExit, "TAR archive member name exceeds"):
                VALIDATOR.require_no_model_artifacts(source)

    def test_rejects_a_zip_archive_with_an_oversized_member_name(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            source = Path(temporary_directory)
            with zipfile.ZipFile(source / "archive.zip", "w") as archive:
                archive.writestr(
                    "a" * (VALIDATOR.MAX_ARCHIVE_MEMBER_NAME_BYTES + 1), "source only"
                )
    ...
    def test_rejects_an_ar_archive_with_an_oversized_member_name(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            source = Path(temporary_directory)
            member_name = "a" * (VALIDATOR.MAX_ARCHIVE_MEMBER_NAME_BYTES + 1)
            (source / "archive.a").write_bytes(
                VALIDATOR.AR_MAGIC + ar_bsd_member(member_name, b"source only")
            )
