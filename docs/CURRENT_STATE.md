...
progress. M56-G6 is complete: closed Tauri template lifecycle/application
commands and a strict TypeScript/Zod bridge now expose only bounded visible
projections, opaque native concurrency handles, and reservation outcomes. The
next action is M56-G7: template-management UI. No UI, packaging, installation,
tag, or release is claimed by this checkpoint.
