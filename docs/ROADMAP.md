...
M69 makes Threads the primary QuireForge interaction: a normal, no-project,
local-only conversation whose first destination is the approved bounded local
runtime. Work and Code become explicit capability states inside a thread rather
than peer lanes. Files, projects, artifacts, Git, terminal, tools, and optional
providers remain explicit conversational actions rather than hidden context.
...
[M71 -- Evidence Linkage](MILESTONE_71_EVIDENCE_LINKAGE.md) is validated in
...
context-transfer, provider, or execution capability. Full source validation,
desktop and website E2E, the clean pinned Linux package/release-artifact gate,
and visible Debian-package launch have passed; owner acceptance remains
separate.

[M76 -- Isolated Read-Only Browser Research](MILESTONE_76_ISOLATED_BROWSER_RESEARCH.md)
has a `0.1.0-beta.103` replacement candidate. Beta.102 passed source,
browser-E2E, clean-package, passwordless-installation, and bounded
installed-host-helper checks but failed owner UI acceptance because its review
was mounted in a hidden workspace. Beta.103 mounts the fixed Google-only review
in visible Local Chat and preserves the separate prepare/confirm/revoke flow;
package and installed-host UI acceptance remain pending.
